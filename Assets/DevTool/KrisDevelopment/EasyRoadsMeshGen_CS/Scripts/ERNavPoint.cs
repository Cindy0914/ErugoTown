...
namespace KrisDevelopment.ERMG
{
	...
	public class ERNavPoint : MonoBehaviour
	{
		...
		public float lockedHalfWidth = 1f;
		...
		public void NavPointAction(NavAction act)
		{
			...
			{
				Debug.LogError("[ERROR ERMG.ERNavPoint] No assigned MeshGen at " + name);
				return;
			}
		...
		public void LockSize(bool state)
		{
			...
			{
				SerializedProperty so_lockSize = so.FindProperty(nameof(lockSize));
				SerializedProperty so_lockedWidth = so.FindProperty(nameof(lockedHalfWidth));

				if (!assignedMeshGen)
				{
					Debug.LogError("[ERROR ERMG.ERNavPoint] No MeshGen script assigned! Lock operation failed!");
					so_lockSize.boolValue = false;
				}
				else
				{
					so_lockSize.boolValue = state;
					so_lockedWidth.floatValue = assignedMeshGen.halfDeltaWidth * transform.localScale.x;
				}
		...
		public void SetIndex(int index, ERMeshGen inMeshGen)
		{
			assignedMeshGen = inMeshGen;
#if UNITY_EDITOR
			using (SerializedObject so = new SerializedObject(this))
			{
				SerializedProperty so_index = so.FindProperty(nameof(lastKnownIndex));

				if (!assignedMeshGen)
				{
					Debug.LogError("[ERROR ERMG.ERNavPoint] No MeshGen script assigned!");
					so_index.intValue = -1;
				}
				else
				{
					so_index.intValue = index;
				}
